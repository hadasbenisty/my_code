function plotTilingByBehaveD16(meanDataLog, X_N, eventNameList, behaveDataProc, roiLocs, neurons_order, time_order, solutionTiling, byttl, timevec, animalName, behave_events_times, allLabels,avifilename)
...
% end
% allbehavehist = hist(allLabels.trialBehave', 0:14);
% allbehavehist = allbehavehist([2     3     4     5     6     7     8     9    14],:);
% 
% figure;
...
% legend({'Lift','Grab','Sup','Atmouth','Chew','Sniff','Handopen','Botharm', 'Trying'}); % (do not touch));
% suptitle(byttl);
...
% linkaxes(a, 'x');legend({'Lift','Grab','Sup','Atmouth','Chew','Sniff','Handopen','Botharm', 'Trying'}); % (do not touch));
...
%     subplot(2,length(meanDataLog_all),k)
    subplot(2,ceil(length(meanDataLog_all)/2),k)
...
suptitle([ animalName ' All Exps Tiled By ' byttl]);
% figure;
% for k  = 1:length(meanDataLog_all)
...
% behavehist = behavehist([2     3     4     5     6     7     8     9    14],:);
% 
% 
%     a(k) = subplot(2,length(meanDataLog_all),k);
%     mat2display = getTiledData(meanDataLog_all{k}.alltrials, solutionTiling);

function [data, wires, x, f] = loadVcdData(datapth, uniqueX, uniqueZ, Nparams_all, NparamsX, Yvals, NparamsZ, Mvals, overwrite, isbin2dec)
...
if ~isbin2dec
    filenameout = [filenameout '_x_' num2str(NparamsX) 'vals_z_' num2str(NparamsZ) 'valsBin.mat'];
else
filenameout = [filenameout '_x_' num2str(NparamsX) 'vals_z_' num2str(NparamsZ) 'vals.mat'];
end
        ...
        for M = Mvals
            for X=Xvec
                for Z=Zvec
                    ...
                    [data.samps{n}, wires{n}] = parse_vcd(fullfile(datapth, filenameIn),0, inf, isbin2dec);
                    wires{n}.name = zeros(length(wires{n}.module),1);
                    dict=[];
                    for k=1:length(wires{n}.module)
                        ind = find(strcmp(wires{n}.module{k},dict));
                        if isempty(ind)
                            dict{end+1} =   wires{n}.module{k};
                            wires{n}.name(k) = length(dict);
                        else
                            wires{n}.name(k) =ind;
                        end
                    end
...
for n=1:length(data.samps)
    if length(data.samps{n})~=441%166
        f(n)=0;
    else
